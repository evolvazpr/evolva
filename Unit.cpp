...
void Unit::Update() {
	...
	if (turns_ >= death_) field->Kill(GetUnit());
	if (fatigue_ >= dna_["fatigue_death_threshold"]) field->Kill(GetUnit());
	if (poison_ > 0.0) health_ -= dna_["poison_susceptibility"] * poison_;
	if (health_ <= dna_["health_death_threshold"]) field->Kill(GetUnit());

	// attractiveness
	attractiveness_ = (dna_["speed"] + dna_["intelligence"] + dna_["strength"] + dna_["agility"]) * (0.5 + health_ / 200.0);

	if (sleep_) {
		if ((energy_ -= dna_["requirements.sleep_turn"]) < 0.0) field->Kill(GetUnit());
		if (poison_ > 0.0) {
		...
		if ((energy_ -= dna_["requirements.normal_turn"]) < 0.0) field->Kill(GetUnit());
		if (poison_ > 0.0) {
...
size_t Unit::Think(const double intelligence, std::shared_ptr<Unit> attacker) {
	...
	if (!!attacker) {
		...
		if (target_health < 0.0) field->Kill(GetUnit());
	...
	// Other cases: explore.
	return Explore(std::min(M_SQRT2 * 3.0, steps_limit));
}
...
bool Unit::Move(const int x, const int y, const bool trim) {
	...
	if (energy_ >= cost) {
		energy_ -= cost;
		return MovableObject::Move(x, y, trim);
	}
	else return false;
}
...
void Unit::GiveBirth(const int x, const int y) {
	...
	std::cout << "\n";
	energy_ -= dna_["requirements.childbirth"];
	pregnant_ = false;
	pregnant_turns_ = 0;
