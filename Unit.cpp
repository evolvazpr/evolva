...
Unit::Unit(std::shared_ptr<DnaCode> dna_code) : DnaUnit(dna_code), CellObject() {
	try {
		...
		logger << "DNA data is corrupted. Cannot create Unit: " << e.what() << "\n";
		throw e;
	}
}
...
void Unit::Sleep() {
	sleep_ = true;
	logger << "Unit " << GetId() << " fell asleep\n";
}

void Unit::WakeUp() {
	sleep_ = false;
	logger << "Unit " << GetId() << " woke up\n";
}
...
size_t Unit::Think(const double intelligence, std::shared_ptr<Unit> attacker) {
    ...
    // Mating if not pregnant.
		...
		if(best_cell) {
			...
			logger << "Mating: " << GetId() << " with " << unit->GetId() << "\n";
...
void Unit::GiveBirth(const int x, const int y) {
	...
	logger << "Unit " << GetId() << " gives a birth to " << child->GetId() << " at " << child->GetX() << ", " << child->GetY() << " (" << GetX() + x << ", " << GetY() + y << ")\n";
